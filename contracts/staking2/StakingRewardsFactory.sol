...
contract StakingRewardsFactory is Ownable {
    ...
    uint public delayToPullFLIQ = 259200;
    address public pullFLIQTo; 
    uint public requestedAmount;
    uint public requestTimestamp;
    bool public isPullRequested;
    ...
    function requestPullFLIQ(uint _amount,address _recipient) external onlyOwner {
        require(!isPullRequested, "Pull FLIQ already requested");
        require(_amount > 0 && _amount <= IERC20(rewardsToken).balanceOf(address(this)), "Wrong amount");
        pullFLIQTo = _recipient;
        requestedAmount = _amount;
        isPullRequested = true;
        requestTimestamp = block.timestamp;
    }
    
    function abortPullFLIQ() external onlyOwner {
        require(isPullRequested, "Pull FLIQ not requested"); 
        isPullRequested = false;
    }

    function pullFLIQ() external onlyOwner {
        require(isPullRequested, "Pull FLIQ not requested");
        require(block.timestamp > requestTimestamp + delayToPullFLIQ, "Too early");
        isPullRequested = false;
        IERC20(rewardsToken).transfer(pullFLIQTo, requestedAmount);
    }
}